...
def clear_names(file_name: str) -> list:  # принимаем строки, возвращаем список имен
    ...
    with open(
        "data/" + file_name
    ) as names_file:  # с помощью контекстного менеджера открываем файл names.txt + прибавляем файл кот мы принимаем
        names_list = (
            names_file.read().split()
        )  # читаем файл целиком и разделяем на строки. получим массив из списка имен в 112 строк
...
def is_cyrillic(name_item: str) -> bool:
    ...
    return bool(
        re.search("[а-яА-Я]", name_item)
    )  # импортируем из библиотеки регулярных выражений. в [] задаем паттерн
